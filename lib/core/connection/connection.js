...
const OP_COMPRESSED = shared_1.opcodes.OP_COMPRESSED;
const OP_MSG = shared_1.opcodes.OP_MSG;
// TS-TODO
...
class Connection extends events_1.EventEmitter {
    ...
    constructor(socket, options) {
        ...
        if (this.logger.isDebug()) {
            this.logger.debug(`creating connection ${this.id} with options [${JSON.stringify(utils_1.debugOptions(DEBUG_FIELDS, options) // TS-TODO
            )}]`);
        }
        // Response options
...
function errorHandler(conn) {
    return function (err) {
        ...
        conn.emit('error', new error_1.MongoNetworkError(err), conn);
    };
}
function timeoutHandler(conn) {
    return function () {
        ...
        conn.emit('timeout', new error_1.MongoNetworkError(`connection ${conn.id} to ${conn.address} timed out`), conn);
    };
}
function closeHandler(conn) {
    return function (hadError) {
        ...
        if (!hadError) {
            conn.emit('close', new error_1.MongoNetworkError(`connection ${conn.id} to ${conn.address} closed`), conn);
        }
    };
}
// Handle a message once it is received
function processMessage(conn, message) {
    const msgHeader = shared_1.parseHeader(message);
    if (msgHeader.opCode !== OP_COMPRESSED) {
        const ResponseConstructor = msgHeader.opCode === OP_MSG ? msg_1.BinMsg : commands_1.Response;
        conn.emit('message', new ResponseConstructor(conn.bson, message, msgHeader, message.slice(shared_1.MESSAGE_HEADER_SIZE), conn.responseOptions), conn);
        return;
    }
    msgHeader.fromCompressed = true;
    let index = shared_1.MESSAGE_HEADER_SIZE;
    ...
    decompress(compressorID, message.slice(index), (err, decompressedMsgBody) => {
        ...
        if (decompressedMsgBody.length !== msgHeader.length) {
            conn.emit('error', new error_1.MongoError('Decompressing a compressed message from the server failed. The message is corrupt.'));
            return;
        }
        const ResponseConstructor = msgHeader.opCode === OP_MSG ? msg_1.BinMsg : commands_1.Response;
        ...
        while (data.length > 0) {
            ...
            if (conn.bytesRead > 0 && conn.sizeOfMessage > 0) {
                ...
                if (remainingBytesToRead > data.length) {
                    ...
                    data = safe_buffer_1.Buffer.alloc(0);
                }
                else {
                ...
                if (conn.stubBuffer != null && conn.stubBuffer.length > 0) {
                    ...
                    if (conn.stubBuffer.length + data.length > 4) {
                        // Prepad the data
                        const newData = safe_buffer_1.Buffer.alloc(conn.stubBuffer.length + data.length);
                        ...
                        const newStubBuffer = safe_buffer_1.Buffer.alloc(conn.stubBuffer.length + data.length);
                    ...
                    if (data.length > 4) {
                        ...
                        if (sizeOfMessage > 4 &&
                            ...
                            conn.buffer = safe_buffer_1.Buffer.alloc(sizeOfMessage);
                        ...
                        conn.stubBuffer = safe_buffer_1.Buffer.alloc(data.length);
