...
const command_result_1 = require("./command_result");
const shared_1 = require("../wireprotocol/shared");
...
class Pool extends events_1.EventEmitter {
    constructor(topology, options) {
        ...
        this.logger = new logger_1.Logger("Pool", options);
        // event handlers
        const pool = this;
    ...
    connect() {
        if (this.state !== DISCONNECTED) {
            throw new error_1.MongoError("connection in unlawful state " + this.state);
        }
        const self = this;
        stateTransition(this, CONNECTING);
        self.connectingConnections++;
        connect_1.connect(self.options, (err, connection) => {
            self.connectingConnections--;
            // TS-TODO: type guard?
            if (err) {
        ...
        if (force) {
            ...
            while (self.queue.length > 0) {
                ...
                if (typeof workItem.cb === "function") {
                    workItem.cb(new error_1.MongoError("Pool was force destroyed"));
                }
            }
    ...
    reset(callback) {
        ...
        utils_1.eachAsync(connections, (conn, cb) => {
    ...
    write(command, options, cb) {
        ...
        if (!(typeof cb === "function") && !options.noResponse) {
            throw new error_1.MongoError("write method must provide a callback");
        }
        ...
        if (this.state === DESTROYED || this.state === DESTROYING) {
            ...
            if (cb) {
                try {
                    cb(new error_1.MongoError("pool destroyed"));
                }
                catch (err) {
...
function stateTransition(self, newState) {
    ...
    if (legalStates && legalStates.indexOf(newState) !== -1) {
        ...
        self.logger.error(util_1.format("Pool with id [%s] failed attempted illegal state transition from [%s] to [%s] only following state allowed [%s]", self.id, self.state, newState, legalStates));
    }
}
        ...
        connect_1.connect(self.options, (err, connection) => {
            ...
            if (err) {
                ...
                if (self.retriesLeft <= 0) {
                    self.destroy();
                    self.emit("reconnectFailed", new error_1.MongoNetworkError(util_1.format("failed to reconnect after %s attempts with interval %s ms", self.options.reconnectTries, self.options.reconnectInterval)));
                }
                else {
...
function messageHandler(self) {
    return function (message, connection) {
        ...
        if (self.logger.isDebug()) {
            self.logger.debug(util_1.format("message [%s] received from %s:%s", message.raw.toString("hex"), self.options.host, self.options.port));
        }
        ...
        if (workItem && !workItem.immediateRelease) {
            try {
                ...
                return handleOperationCallback(self, workItem.cb, new error_1.MongoError(err));
            }
            ...
            if (workItem.command && message.documents[0]) {
                ...
                if (responseDoc.writeConcernError) {
                    const err = new error_1.MongoWriteConcernError(responseDoc.writeConcernError, responseDoc);
                ...
                if (responseDoc.ok === 0 ||
                    ...
                    responseDoc.errmsg ||
                    responseDoc.code) {
                    return handleOperationCallback(self, workItem.cb, new error_1.MongoError(responseDoc));
                }
            }
            // Add the connection details
            message.hashedName = connection.hashedName;
            // Return the documents
            handleOperationCallback(self, workItem.cb, null, new command_result_1.CommandResult(workItem.fullResult ? message : message.documents[0], connection, message));
        }
    };
}
function totalConnectionCount(pool) {
    return (pool.availableConnections.length +
...
function serializeCommand(self, command, callback) {
    ...
    const concatenatedOriginalCommandBuffer = safe_buffer_1.Buffer.concat(originalCommandBuffer);
    const messageToBeCompressed = concatenatedOriginalCommandBuffer.slice(shared_1.MESSAGE_HEADER_SIZE);
    ...
    compress(self, messageToBeCompressed, function (err, compressedMessage) {
        ...
        const msgHeader = safe_buffer_1.Buffer.alloc(shared_1.MESSAGE_HEADER_SIZE);
        msgHeader.writeInt32LE(shared_1.MESSAGE_HEADER_SIZE + shared_1.COMPRESSION_DETAILS_SIZE + compressedMessage.length, 0); // messageLength
        msgHeader.writeInt32LE(command.requestId, 4); // requestID
        msgHeader.writeInt32LE(0, 8); // responseTo (zero)
        msgHeader.writeInt32LE(shared_1.opcodes.OP_COMPRESSED, 12); // opCode
        // Create the compression details of OP_COMPRESSED
        const compressionDetails = safe_buffer_1.Buffer.alloc(shared_1.COMPRESSION_DETAILS_SIZE);
...
function canCompress(command) {
    const commandDoc = command instanceof msg_1.Msg ? command.command : command.query;
...
function flushMonitoringOperations(queue) {
    for (var i = 0; i < queue.length; i++) {
        if (queue[i].monitoring) {
            ...
            queue.splice(i, 1);
            workItem.cb(new error_1.MongoError({
