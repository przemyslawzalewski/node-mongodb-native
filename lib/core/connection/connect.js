...
const constants_1 = require("../wireprotocol/constants");
const createClientInfo = require('../topologies/shared').createClientInfo;
let AUTH_PROVIDERS;
function connect(options, callback) {
    if (AUTH_PROVIDERS == null) {
        AUTH_PROVIDERS = defaultAuthProviders_1.defaultAuthProviders(options.bson);
    }
    if (options.family !== void 0) {
        makeConnection(options.family, options, (err, socket) => {
            ...
            performInitialHandshake(new connection_1.Connection(socket, options), options, callback);
        });
        return;
    }
    return makeConnection(6, options, (err, ipv6Socket) => {
        if (err) {
            makeConnection(4, options, (err, ipv4Socket) => {
                ...
                performInitialHandshake(new connection_1.Connection(ipv4Socket, options), options, callback);
            });
            return;
        }
        performInitialHandshake(new connection_1.Connection(ipv6Socket, options), options, callback);
    });
}
...
function checkSupportedServer(ismaster, options) {
    ...
    if (serverVersionHighEnough) {
        ...
        const message = `Server at ${options.host}:${options.port} reports minimum wire version ${ismaster.minWireVersion}, but this version of the Node.js Driver requires at most ${constants_1.MAX_SUPPORTED_WIRE_VERSION} (MongoDB ${constants_1.MAX_SUPPORTED_SERVER_VERSION})`;
        return new error_1.MongoError(message);
    }
    const message = `Server at ${options.host}:${options.port} reports maximum wire version ${ismaster.maxWireVersion ||
        0}, but this version of the Node.js Driver requires at least ${constants_1.MIN_SUPPORTED_WIRE_VERSION} (MongoDB ${constants_1.MIN_SUPPORTED_SERVER_VERSION})`;
    return new error_1.MongoError(message);
}
function performInitialHandshake(conn, options, _callback) {
    ...
    runCommand(conn, 'admin.$cmd', handshakeDoc, options, (err, ismaster) => {
        ...
        if (ismaster.ok === 0) {
            callback(new error_1.MongoError(ismaster), null);
            return;
        }
...
function makeConnection(family, options, _callback) {
    ...
    try {
        if (useSsl) {
            socket = tls_1.connect(parseSslOptions(family, options));
            if (typeof socket.disableRenegotiation === 'function') {
                socket.disableRenegotiation();
            }
        }
        else {
            socket = net_1.createConnection(parseConnectOptions(family, options));
        }
    }
    catch (err) {
        return callback(err);
...
function runCommand(conn, ns, command, options, callback) {
    ...
    const query = new commands_1.Query(bson, ns, command, {
        numberToSkip: 0,
        numberToReturn: 1
    });
...
function authenticate(conn, credentials, callback) {
    ...
    if (!AUTH_PROVIDERS[mechanism]) {
        callback(new error_1.MongoError(`authMechanism '${mechanism}' not supported`));
        return;
    }
...
function connectionFailureError(type, err) {
    switch (type) {
        ...
        case 'timeout':
            return new error_1.MongoNetworkError(`connection timed out`);
        case 'close':
