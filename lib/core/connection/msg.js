...
class Msg {
    constructor(bson, ns, command, options) {
        ...
        this.ns = ns;
        this.command = command;
        this.command.$db = shared_1.databaseNamespace(ns);
    ...
    toBin() {
        ...
        const header = new Buffer(4 * 4 + // Header
            4 // Flags
        );
        buffers.push(header);
        ...
        header.writeInt32LE(shared_1.opcodes.OP_MSG, 12); // opCode
...
class BinMsg {
    ...
    parse(options) {
        ...
        const promoteBuffers = typeof options.promoteBuffers === 'boolean'
