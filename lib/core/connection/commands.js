...
const Long = utils_1.retrieveBSON().Long;
...
class Query {
    ...
    toBin() {
        ...
        // TS-TODO check this?
        var header = safe_buffer_1.Buffer.alloc(4 * 4 + // Header
            4 + // Flags
            safe_buffer_1.Buffer.byteLength(self.ns) +
            1 + // namespace
            4 + // numberToSkip
            4 // numberToReturn
        );
        ...
        header[index + 3] = (shared_1.opcodes.OP_QUERY >> 24) & 0xff;
        header[index + 2] = (shared_1.opcodes.OP_QUERY >> 16) & 0xff;
        header[index + 1] = (shared_1.opcodes.OP_QUERY >> 8) & 0xff;
        header[index] = shared_1.opcodes.OP_QUERY & 0xff;
        index = index + 4;
...
class GetMore {
    ...
    // Uses a single allocated buffer for the process, avoiding multiple memory allocations
    toBin() {
        var length = 4 + safe_buffer_1.Buffer.byteLength(this.ns) + 1 + 4 + 8 + 4 * 4;
        // Create command buffer
        var index = 0;
        // Allocate buffer
        var _buffer = safe_buffer_1.Buffer.alloc(length);
        ...
        _buffer[index + 3] = (shared_1.opcodes.OP_GETMORE >> 24) & 0xff;
        _buffer[index + 2] = (shared_1.opcodes.OP_GETMORE >> 16) & 0xff;
        _buffer[index + 1] = (shared_1.opcodes.OP_GETMORE >> 8) & 0xff;
        _buffer[index] = shared_1.opcodes.OP_GETMORE & 0xff;
        index = index + 4;
        // index = write32bit(index, _buffer, 0);
        _buffer[index + 3] = (0 >> 24) & 0xff;
        _buffer[index + 2] = (0 >> 16) & 0xff;
...
class KillCursor {
    ...
    toBin() {
        ...
        _buffer[index + 3] = (shared_1.opcodes.OP_KILL_CURSORS >> 24) & 0xff;
        _buffer[index + 2] = (shared_1.opcodes.OP_KILL_CURSORS >> 16) & 0xff;
        _buffer[index + 1] = (shared_1.opcodes.OP_KILL_CURSORS >> 8) & 0xff;
        _buffer[index] = shared_1.opcodes.OP_KILL_CURSORS & 0xff;
        index = index + 4;
        // index = write32bit(index, _buffer, 0);
        _buffer[index + 3] = (0 >> 24) & 0xff;
        _buffer[index + 2] = (0 >> 16) & 0xff;
        ...
        for (var i = 0; i < this.cursorIds.length; i++) {
            const cursorId = this.cursorIds[i];
            // Write cursor id
